...
def login():
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(user_identifiant=form.identifiant.data).first()

        if user is None:
            user = User.query.filter_by(user_email=form.identifiant.data).first()

            if user is None:
                flash("Identifiant et/ou mot de passe incorrect(s)")
    ...
    if current_user.is_authenticated:
        ...
        if session.get("user_email") is None:
            ...
            return render_template(
                "register.html",
                more_infos_form=more_infos_form,
...
def register_password():
    ...
    if password_form.validate_on_submit():
        user_pic = random.choice(os.listdir("static/images/"))
